...
const CARD_IMG =[
    'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine', 'ten', 'eleven', 'towelve', 'one1', 'two1', 'three1', 'four1', 'five1', 'six1', 'seven1', 'eight1', 'nine1', 'ten1', 'eleven1', 'towelve1'
];

const BOARD_SIZE = 24; //카드 총 개수
...
function clearStage() {
    ...
    // 20초 이하로는 빨라지지 않음
    if (stage <= 8) {
        time = 80 - (stage * 5); // 남은 시간 초기화 (스테이지 진행 시 마다 5초씩 감소)
    }
    stage++; // 스테이지 값 1 추가
    cardDeck = [];
...
function makeCardDeck() {
    // 이미지는 24개인데 필요한 카드는 12개로 고정되어 있기 때문에 24개의 이미지 중 랜덤으로 12개를 뽑도록 구현
    let randomNumberArr = [];

    for (let i = 0; i < BOARD_SIZE / 2; i++) {
        // 랜덤 값 뽑기
        let randomNumber = getRandom(24, 0);
...
// 카드 화면에 세팅
const gameBoard = document.getElementsByClassName("game_board")[0];
...
function settingCardDeck() {
    for (let i = 0; i < BOARD_SIZE; i++) {
        ...
        cardFront[i].style.backgroundImage = `url('./images/${cardDeck[i].card}.png')`;
    }
}
...
function showGameResult() {
    ...
    <h1 class="modal_content-title--result color-red">
        게임 종료!
    </h1>
    <span class="modal_content-title--stage">
        기록 : <strong>STAGE ${stage}</strong>
    </span>
    <p class="modal_content-title--desc">
        ${resultText}
    </p>
    `;
...
// 모달창 닫으면 게임 재시작
const modalTitle = document.getElementsByClassName("modal_content-title")[0];
const modalCloseButton = document.getElementsByClassName("modal_content-close-button")[0];
